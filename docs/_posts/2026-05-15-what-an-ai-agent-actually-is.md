...
summary: Strip away the marketing and an agent is four parts — a driver, tools, an environment, and injected context. Name the broken part and the fix is usually obvious.
---
...
Strip away the marketing and an AI agent is four things glued together: a **driver**, a set of **tools**, an **environment**, and **injected context**. Once you see it that way, the architecture of any given agent — from a Claude Code session to a cron-driven scraper to a phone-based assistant — becomes a lot easier to reason about.

One terminology rule up front, because the fourth component is invisible without it: the **environment** is where the compute happens, and **context** means exactly one thing — the context window of the model you're talking to in the moment. Keep those words apart and the rest of this falls into place.
...
This is the part most people gloss over, and it's where the actual agent lives. A driver, a tool set, and an environment interoperate — but why would they do anything at all? Nothing happens until something puts tokens in front of the driver, and everything that does is **context injection**.

The first injection is the trigger, and it's usually one of:
...
But the trigger is never the whole injection. When you type a sentence into Claude Code, the string that actually reaches the model is your message *plus* a system prompt, the tool definitions, and whatever memory the harness maintains — those markdown files it re-reads and passes in every single time. And on the way back out, the harness decides which of the returned tokens are interpreted as tool use and executed, and which are passed back to you. Every harness answers this differently; that assembly-and-interpretation job is **context management**, and it's most of what an agent framework actually is. The features wear other names — *memory* is context management at the persistence layer; *skills* are context injection via markdown pre-prompts ([Anthropic 2025b](https://www.anthropic.com/news/skills)) — but it's the same job underneath.

Once triggered, the run usually looks like some variant of the ReAct pattern ([Yao et al. 2022](https://arxiv.org/abs/2210.03629)): the driver produces a thought, picks an action (a tool call), observes the result, and feeds the observation back into its context for the next iteration. That loop *is* context injection on repeat — each observation is one more injection — and it continues until the driver decides the task is done, or until something external stops it.
...
That's the entire pattern — note that `context.append` is doing the load-bearing work. Everything else — multi-agent orchestration, planning, evaluator/optimizer loops, the whole menagerie of patterns Anthropic catalogs in their agents post ([Anthropic 2024](https://www.anthropic.com/engineering/building-effective-agents)) — is a variation on how context flows into the driver, or a way of composing multiple such flows together.
...
- **Goes off the rails?** That's usually the context. No stopping condition, no checkpoints, the wrong things injected — or context accumulating until the driver forgets what it was doing.
...
Agents aren't magic, and they aren't a single thing. They're a driver, a set of tools, an environment, and an injected context that makes anything happen at all. Build each one deliberately and the system mostly takes care of itself.

I push this decomposition further — multi-agent composition, and tracking agents by the actions they take rather than the outputs they produce — in [The Agent Landscape]({% post_url 2026-06-10-the-agent-landscape %}).
...
- Anthropic. 2025a. "Code Execution with MCP: Building More Efficient Agents." Anthropic. <https://www.anthropic.com/engineering/code-execution-with-mcp>.
- Anthropic. 2025b. "Introducing Agent Skills." Anthropic. <https://www.anthropic.com/news/skills>.
